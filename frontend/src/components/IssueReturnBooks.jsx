...
const IssueReturnBooks = ({ onStatsUpdate }) => {
  ...
  const handleReturnBook = async (transactionId) => {
    ...
    try {
      ...
      const fineMsg = response.data.fineAmount > 0 
        ? ` with fine: ₹${response.data.fineAmount}` 
        : '';
  ...
  const getSelectedStudent = () => {
    return students.find(s => s._id === issueForm.studentId);
  };

  const getSelectedBook = () => {
    return books.find(b => b._id === issueForm.bookId);
  };

  const isStudentAtLimit = () => {
    const student = getSelectedStudent();
    return student && student.currently_borrowed_books >= student.max_books_allowed;
  };
                ...
                <Grid container spacing={2}>
                  <Grid size={{ xs: 12 }}>
                    ...
                    >
                      ...
                      {studentsLoading ? (
                        <MenuItem disabled>
                          <CircularProgress size={20} /> Loading students...
                        </MenuItem>
                      ) : (
                  ...
                  <Grid size={{ xs: 12 }}>
                    <TextField
                      ...
                      onChange={(e) => setIssueForm({...issueForm, dueDays: parseInt(e.target.value)})}
                      fullWidth
                      required
                  ...
                  {issueForm.studentId && (
                    <Grid size={{ xs: 12 }}>
                      <Alert 
                        severity={isStudentAtLimit() ? "error" : "info"}
                        sx={{ mt: 1 }}
                      >
                        <Typography variant="body2">
                          ...
                          Borrowed: {getSelectedStudent()?.currently_borrowed_books || 0}/{
                            getSelectedStudent()?.max_books_allowed || 3
                          } books
                  ...
                  <Grid size={{ xs: 12 }}>
                    <Button
                      ...
                      variant="contained"
                      fullWidth
                      disabled={loading || !issueForm.studentId || !issueForm.bookId || isStudentAtLimit() || students.length === 0}
                    >
                  ...
                  <TableBody>
                    {issuedBooks.map((transaction) => (
                      <TableRow key={transaction._id}>
                        ...
                        <TableCell>
                          <Chip
                            label={new Date(transaction.due_date).toLocaleDateString()}
                            color={isOverdue(transaction.due_date) ? "error" : "default"}
                            size="small"
                          />
                        </TableCell>
                        <TableCell>
                          <Typography 
                            color={calculateFine(transaction.due_date) > 0 ? "error" : "textSecondary"}
                            variant="body2"
                          >
                            ₹{calculateFine(transaction.due_date)}
                          </Typography>
                        </TableCell>
                        <TableCell>
                          <Button
