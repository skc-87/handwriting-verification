...
const faceController = {
  ...
  registerFace: async (req, res) => {
    ...
    try {
      ...
      if (!fs.existsSync(pythonScript)) {
        throw new Error(
          "Face recognition system not found at: " + pythonScript
        );
      }
      ...
      console.log(
        `🔍 Executing: python ${pythonScript} register ${student_id} [name] [image_path] [token_redacted]`
      );
      ...
      const result = await new Promise((resolve, reject) => {
        const child = execFile(
          ...
          (error, stdout, stderr) => {
            if (error) {
              console.error(
                `🐍 Python Error (${error.code}): ${stderr || error.message}`
              );
              const errorMsg =
  ...
  takeAttendance: async (req, res) => {
    ...
    try {
      ...
      console.log(
        `🔍 Executing: python ${pythonScript} attendance ${subject} [image_path] ${date} [token_redacted]`
      );

      // Replace the execFile promise part with this:
      const result = await new Promise((resolve, reject) => {
        const child = execFile(
          "python",
          [pythonScript, ...args],
          { timeout: 30000 },
          (error, stdout, stderr) => {
            // IGNORE stderr completely - it's just TensorFlow warnings
            // Only check stdout for valid JSON
            try {
              const output = stdout.trim();
              if (!output) {
                reject(new Error("No response from attendance system"));
                return;
              }
              ...
              console.error("Failed to parse Python output:", stdout);
              // Even if we got stderr, still try to parse if there's JSON
              if (stdout) {
                try {
                ...
                reject(new Error("Attendance system failed to respond"));
              }
            }
          }
        );
        ...
        child.on("exit", (code) => {
          // Don't treat non-zero exit as error since we handle it in stdout
